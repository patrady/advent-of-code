import DayEight from "./day_eight";
...
const realInput = `daegb gadbcf cgefda edcfagb dfg acefbd fdgab fg bdcfa fcgb | cdfgba fgbc dbfac gfadbc
...
describe("DayEight", () => {
  describe("partOne", () => {
    it("works with sample input", () => {
      const program = new DayEight(sampleInput);

      expect(program.partOne()).toEqual(26);
    });

    it("works with real input", () => {
      const program = new DayEight(realInput);

      expect(program.partOne()).toEqual(493);
    });
  });
